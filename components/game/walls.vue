...
.walls-container
  div(v-for="(wall, index) in walls" :key="index")
    img(:src="get_wall_path('wall-left.png')" :style="{ height: '50vh', left: 0, top: `${wall.value.y}px`, position: 'absolute' }")
    img(:src="get_wall_path('wall-right.png')" :style="{ height: '50vh', right: 0, top: `${wall.value.y}px`, position: 'absolute' }")
</template>
